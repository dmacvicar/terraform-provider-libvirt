...
func rebootDomainWhenCreated() {
	//TODO: implement This
	// wait with goroutine channel until the domain is created then reboot domain
}

func TestAccLibvirtDomain_RebootEvent(t *testing.T) {
	randomResourceName := acctest.RandString(10)
	randomDomainName := acctest.RandString(10)
	randomKernelName := acctest.RandString(10)
	randomInitrdName := acctest.RandString(10)
	resource.Test(t, resource.TestCase{
		// TODO: 1) initialize the domain_up_and_running event (create) and listner.
		//  The callback should reboot the domain.
		PreCheck:     rebootDomainWhenCreated,
		Providers:    testAccProviders,
		CheckDestroy: testaccCheckLibvirtDestroyResource("libvirt_domain", *testAccProvider.Meta().(*Client).libvirt),
		...
		Steps: []resource.TestStep{
			{
				...
				}`, randomKernelName, randomInitrdName, randomResourceName, randomDomainName),
				Check: resource.ComposeTestCheckFunc(
				// we just need one step here.
				// TODO: 2) this tests will wait forever if reboot event is not caught.
				// additionaly check that kernel and initrd + cmdline params are empty.
				// thx to the precheck function we will reboot the domain and then run the callback test.
				),
			},
		},
	})
