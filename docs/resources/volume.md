...
- `capacity` (Number) Volume capacity in bytes (required unless using create.content, source.host_path, or clone)
...
- `clone` (Attributes) Clone options for creating a volume by cloning an existing volume (see [below for nested schema](#nestedatt--clone))
- `source` (Attributes) Source options for adopting an existing file on the libvirt host as a volume (see [below for nested schema](#nestedatt--source))
...
- `host_path` (String) Absolute path to an existing file on the libvirt host (e.g. /var/lib/libvirt/images/ubuntu.qcow2). The file is adopted as a volume in the pool via pool refresh.
...
- `volume` (String) Name of the source volume to clone from
- `pool` (String) Name of the pool containing the source volume to clone from

Optional:

- `full_clone` (Boolean) When true, perform a full copy (default). When false, use reflink (copy-on-write) if the filesystem supports it.
